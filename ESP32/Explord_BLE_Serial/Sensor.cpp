...
Characteristic Humidity( // defined by the BLE GATT
...
void Sensor::init() {
  switch (Id) 
  {
    ...
    case 2 : //LOX02
      {
      Sensor::Name=String("LOX");
      Sensor::CharNb=4;
      Sensor::CharSet[0]=&O2;
      Sensor::CharSet[1]=&Temp;
      Sensor::CharSet[2]=&Pressure;
      Sensor::CharSet[3]=&PPO2;
      }
    case 3 : //MHZ16
      {
      Sensor::Name=String("MHZ");
      Sensor::CharNb=2;
      Sensor::CharSet[0]=&CO2;
      Sensor::CharSet[1]=&Temp;
      }
  ...
  switch (Id)
  {
    ...
    case 2 : //LOX02
      ...
      if(Serial1.available()){ //true if new datas are sended by LOX sensor : vrai si des nouvelles données envoyées par le LOX sont disponibles
        ...
        if (LoxD.charAt(12) == '-'){_Temp = -1 * _Temp;} // deal with the sign if negative T value 
...
void Sensor::configBLEService(BLEService* pService) {
  for (int n=0;n<CharNb;n++) {CharSet[n]->initBLECharacteristic(pService);}
}

void Sensor::printSerialHeader(){
  Serial.print(CharSet[0]->getName());
  for (int n=1;n<CharNb;n++) {Serial.print(String(","+CharSet[n]->getName()));}
  Serial.println("");
}

void Sensor::printSerialData(){
  Serial.print(CharSet[0]->getSValue());
  for (int n=1;n<CharNb;n++) {Serial.print(String(","+CharSet[n]->getSValue()));}
  Serial.println("");  
}
